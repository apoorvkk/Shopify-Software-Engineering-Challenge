...
class TestParseMenusData(unittest.TestCase):
    ...
    def test_valid_existing_menu_info(self):
        ...
        self.assertEqual(products, {1: {'id': 1, 'data': 'House', 'child_ids': [3]}, 2: {'id': 2, 'data': 'Company', 'child_ids': [4]}, 3: {'id': 3, 'data': 'Kitchen', 'parent_id': 1, 'child_ids': [5]}, 4: {
                         'id': 4, 'data': 'Meeting Room', 'parent_id': 2, 'child_ids': [6]}, 5: {'id': 5, 'data': 'Sink', 'parent_id': 3, 'child_ids': [1]}, 6: {'id': 6, 'data': 'Chair', 'parent_id': 4, 'child_ids': []}})
...
class TestConstructMenusUrl(unittest.TestCase):
    def test_valid_page_and_id(self):
        ...
        expected_url = 'https://backend-challenge-summer-2018.herokuapp.com/challenges.json?page=2&id=3'
        self.assertEqual(url, expected_url)

    def test_no_args_provided(self):
        url = construct_menus_url()
        expected_url = 'https://backend-challenge-summer-2018.herokuapp.com/challenges.json?page=1&id=1'
        self.assertEqual(url, expected_url)
    ...
    def test_invalid_page(self):
        with self.assertRaises(ValueError):
          url = construct_menus_url(page='yolo', problem_id=1)

    def test_invalid_id(self):
        with self.assertRaises(ValueError):
          url = construct_menus_url(page=10, problem_id='yolo')
